...
import { getAllTripLeaders, createTripLeader, getTripLeader, updateTripLeader } from "@/miniservices/tripLeaderMiniService";
 ...
 * @returns An HTTP response object
 */
export async function POST(newTripLeader: CreateTripLeaderDTO): Promise<Response> {
    try {
        const session = await getServerSession(authOptions);
        ...
        let member: MemberDTO | null = await getMemberById(newTripLeader.memberId);
        if (member == null) {
            return new Response(`No member with id ${newTripLeader.memberId} exists.`);
        }

        let didCreateTripLeader: boolean = await createTripLeader(newTripLeader);
        if (!didCreateTripLeader) {
            return new Response("Internal Server Error", { status: 500 });
        }

        let createdTripLeader: TripLeaderDTO = await getTripLeader(newTripLeader.memberId);
 ...
 * @returns An HTTP response object
 */
export async function PUT(updatedTripLeader: UpdateTripLeaderDTO): Promise<Response> {
    try {
        const session = await getServerSession(authOptions);
        ...
        let member: MemberDTO | null = await getMemberById(updatedTripLeader.memberId);
        if (member == null) {
            return new Response(`No member with id ${updatedTripLeader.memberId} exists.`);
        }

        let didUpdateTripLeader: boolean = await updateTripLeader(updatedTripLeader);
        if (!didUpdateTripLeader) {
            return new Response("Internal Server Error", { status: 500 });
        }

        let tripLeader: TripLeaderDTO = await getTripLeader(updatedTripLeader.memberId);
