...
export default function Leaderboard(): JSX.Element {
    const [overallLeaderboard, setOverallLeaderboard] = useState<LeaderboardItemT[]>([]);
    ...
    useEffect(() => {
        fetch("/api/leaderboard/total")
            ...
            .then((data) => {
                let processedData: LeaderboardItemT[] = data.map((item: any, index: number) => {
                    return {
                        key: index,
                        name: item.name,
    ...
    return (
        <>
            ...
            <Table aria-label="Example table with dynamic content" className='max-w-[600px]'>
